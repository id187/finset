...
(async()=>{
  ...
  await page.setViewportSize({width:1280,height:800});await page.goto('http://127.0.0.1:5173/?screen=dashboard',{waitUntil:'networkidle'});
  ...
  await b('저축 계획 조정').click();let release;const pending=new Promise(r=>release=r);await page.route('**/api/recovery',async r=>{await pending;await r.continue()});await b('조정한 계획 비교하기').click();await page.getByRole('button',{name:/앞으로 모을 돈이 줄었어요/}).click();release();await page.waitForResponse(r=>r.url().endsWith('/api/recovery'));assert.equal(await page.locator('.compare-amount').count(),0);await page.unroute('**/api/recovery');
  await b('나의 저축 계획').click();await page.goto('http://127.0.0.1:5173/?screen=setup',{waitUntil:'networkidle'});await page.getByLabel('얼마를 모을까요?').fill('18000000');await page.getByLabel('언제까지 모을까요?').selectOption('60');await b('다음으로').click();await b('다음으로').click();await b('맞는 계획 비교하기').click();await page.locator('.product-card').first().waitFor({timeout:60000});await b('이 계획 선택하기').first().click();await b('이 계획 저장하기').click();
  ...
  await b('저축 계획 조정').click();await page.getByText(/상품 만기 이후의/).waitFor();await page.getByLabel('조정 후 납입할 금액').fill('200000');await b('조정한 계획 비교하기').click();await page.getByText('조정 후 예상 원금',{exact:true}).waitFor();await page.getByLabel('계약 변경 승인이 아닌 원금 계획 비교임을 확인했어요.').check();await b('내 계획에 반영하기').click();await b('변경한 계획 저장하기').click();const after=await page.evaluate(()=>JSON.parse(localStorage.getItem('finset-demo-v1')));assert.equal(after.product.maturity,before.product.maturity);assert.ok(after.schedule.filter(s=>s.date>=before.product.maturity).every(s=>s.amount===300000));assert.equal(after.payments.length,0);
